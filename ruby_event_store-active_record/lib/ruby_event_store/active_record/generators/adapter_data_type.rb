  ...
  module ActiveRecord
    class AdapterDataType
      ...
      def call(adapter, data_type)
        raise "unsupported adapter" unless %w[mysql2 postgres sqlite].include?(adapter.downcase)
        raise "MySQL2 doesn't support #{data_type}" if adapter.downcase == "mysql2" && !SUPPORTED_MYSQL_DATA_TYPES.include?(data_type)
        raise "" if adapter.downcase == "postgres" && !SUPPORTED_POSTGRES_DATA_TYPES.include?(data_type)
