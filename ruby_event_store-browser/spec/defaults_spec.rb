...
module RubyEventStore
  RSpec.describe Browser do
    it "takes path from request" do
      ...
      test_client.get('/res/streams/all/relationships/events')

      expect(test_client.parsed_body["links"]).to eq({
        "last" => "http://railseventstore.org/res/streams/all/relationships/events/head/forward/20",
        "next" => "http://railseventstore.org/res/streams/all/relationships/events/#{events[1].event_id}/backward/20"
      })
    end
