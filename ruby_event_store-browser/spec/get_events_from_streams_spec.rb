...
module RubyEventStore
  module Browser
    ::RSpec.describe GetEventsFromStreams do
      ...
      specify "caps the merged page at count" do
      ...
      specify "more? is true when merged streams still hold events below the page boundary" do
        2.times { |i| publish(stream: "a", at: base_time + i) }
        2.times { |i| publish(stream: "b", at: base_time + 10 + i) }

        expect(reader(stream_names: %w[a b], count: 3).more?).to eq(true)
      end
