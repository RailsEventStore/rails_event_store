  ...
  module Browser
    class GetEventsFromStreams
      ...
      def missing_stream_names
        (stream_names - [SERIALIZED_GLOBAL_STREAM_NAME]).select { |name| chunks.fetch(name).empty? }
      end
      ...
      attr_reader :event_store, :stream_names, :cursor, :sort, :count

      def build_page
        rows = stream_names.flat_map { |name| chunks.fetch(name).map { |event| [name, event] } }
        @more = full_chunks.any?
        return group(rows) unless @more

        boundary = full_chunks.map { |name| time_of(chunks.fetch(name).last) }.max
      ...
      def full_chunks
        stream_names.select { |name| chunks.fetch(name).size == count }
      end

      def drained_rows(boundary)
        full_chunks
          .select { |name| time_of(chunks.fetch(name).last) == boundary }
          .flat_map { |name| drain(name, boundary).map { |event| [name, event] } }
      end

      def drain(name, boundary)
        time_sorted(stream_scope(name)).between(boundary..boundary)
      end
      ...
      def group(rows)
        rows
          .group_by(&:last)
          .map { |event, list| [list.map(&:first).uniq, event] }
