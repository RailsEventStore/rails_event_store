...
RSpec.shared_examples :event_repository do |repository_class|
  ...
  specify 'append_to_stream fail if expected version is nil' do
    expect do
      repository.append_to_stream(event = SRecord.new, 'stream', nil)
  ...
  specify 'link_to_stream fail if expected version is nil' do
    ...
    repository.append_to_stream(event = SRecord.new, 'stream', :any)
    expect do
  ...
  specify 'append_to_stream returns self' do
    repository.
      append_to_stream(event = SRecord.new, 'stream', -1).
      append_to_stream(event = SRecord.new, 'stream', 0)
  end
  ...
  specify 'adds an initial event to a new stream' do
    repository.append_to_stream(event = SRecord.new, 'stream', :none)
  ...
  specify 'links an initial event to a new stream' do
    skip unless test_link_events_to_stream
    repository.
      append_to_stream(event = SRecord.new, 'stream', :none).
  ...
  it 'does not have deleted streams with linked events' do
    skip unless test_link_events_to_stream
    repository.
      append_to_stream(e1 = SRecord.new, 'stream', -1).
  ...
  it 'has or has not domain event' do
    ...
    repository.append_to_stream(SRecord.new(event_id: just_an_id), 'stream', -1)
  ...
  it 'reads batch of events from stream forward & backward' do
    ...
    repository.append_to_stream(SRecord.new, 'other_stream', -1)
    events.each.with_index do |event, index|
      repository.append_to_stream(event, 'stream', index - 1)
    end
    repository.append_to_stream(SRecord.new, 'other_stream', 0)
  ...
  specify "reading all existing stream names" do
    repository.append_to_stream(SRecord.new, "test", -1)
    repository.append_to_stream(SRecord.new, "test",  0)
  ...
  specify 'reading particular event' do
    ...
    repository.
      append_to_stream(SRecord.new, "test", -1).
