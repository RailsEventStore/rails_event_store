...
module RubyEventStore
  ::RSpec.describe Projection do
    ...
    specify "warns when Projection.new is used instead of Projection.init" do
      expect do Projection.new end.to output(<<~EOW).to_stderr
    ...
    specify "block must be given to on event handlers" do
      expect do Projection.init.on(MoneyDeposited) end.to raise_error(ArgumentError, "No handler block given")
    end

    it "does not support anonymous events" do
      expect do Projection.init.on(Class.new) { |_state, _event| } end.to raise_error(
        ArgumentError,
    ...
    specify "supports event class remapping" do
      ...
      balance = Projection.init(0).on(MoneyLost) { |state, event| state - event.data[:amount] }.call(event_store.read)
