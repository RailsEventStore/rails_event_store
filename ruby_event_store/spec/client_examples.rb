module RubyEventStore
  RSpec.shared_examples :client do |client_class|

    specify 'publish_event returns :ok when success' do
      client = client_class.new(repository: InMemoryRepository.new)
    ...
    specify 'published event metadata will be enriched by proc execution' do
      client = client_class.new(repository: InMemoryRepository.new, metadata_proc: ->{ {request_id: '127.0.0.1'} })
      event = TestEvent.new
    ...
    specify 'only timestamp set inn metadata when event stored in stream if metadata proc return nil' do
      client = client_class.new(repository: InMemoryRepository.new, metadata_proc: ->{ nil })
      event = TestEvent.new
