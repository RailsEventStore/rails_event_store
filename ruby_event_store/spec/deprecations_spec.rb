...
RSpec.describe RubyEventStore::Deprecations do
  ...
  it "emits warning only once even if called multiple times" do
    described_class.register(:foo, "foo is deprecated")
    expect { 3.times { described_class.warn(:foo) } }.to output("[DEPRECATION] foo is deprecated\n").to_stderr
  end

  it "emits message keyword argument instead of registered message" do
    described_class.register(:foo, "foo registered message")
    expect { described_class.warn(:foo, message: "custom override message") }.to output(
      /custom override message/,
    ).to_stderr
  end

  it "raises KeyError for unregistered key when no message provided" do
    expect { described_class.warn(:unregistered) }.to raise_error(
      KeyError,
      "Deprecation :unregistered not registered. Call Deprecations.register first.",
    )
  end
