  ...
  RSpec.describe SerializedRecord do
    ...
    specify "in-equality" do
      [
        ...
        a = SerializedRecord.new(event_id: one[0], data: one[1], metadata: one[2], event_type: one[3], timestamp: one[4], valid_at: one[5])
        b = SerializedRecord.new(event_id: two[0], data: two[1], metadata: two[2], event_type: two[3], timestamp: two[4], valid_at: two[5])
        c = Class.new(SerializedRecord).new(event_id: one[0], data: one[1], metadata: one[2], event_type: one[3], timestamp: one[4], valid_at: one[5])
