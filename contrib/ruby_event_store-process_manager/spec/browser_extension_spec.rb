...
RSpec.describe RubyEventStore::ProcessManager::BrowserExtension do
  ...
  specify "responds with the styled not found page for streams not following the convention" do
  ...
  specify "serves its stylesheet by convention and links it in the layout" do
    asset_path = "/extension_assets/0/ruby_event_store_process_manager.css"

    expect(web_client.get("/streams/all").body).to include(asset_path)
    expect(web_client.get(asset_path).status).to eq(200)
  end
